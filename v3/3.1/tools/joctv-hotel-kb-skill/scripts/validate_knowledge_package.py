...
# 九类为推荐值；自定义分类规则 (与后台 knowledge_center.py 同源):
# strip 后非空、≤MAX_CATEGORY_LEN 字符、不含 C0/C1 控制字符。
MAX_CATEGORY_LEN = 16
CATEGORY_CTRL_RE = re.compile(r"[\x00-\x1f\x7f-\x9f]")
...
def validate(package) -> list:
    ...
    for index, entry in enumerate(entries):
        ...
        if (
            ...
            or len(category.strip()) > MAX_CATEGORY_LEN
            or CATEGORY_CTRL_RE.search(category.strip())
        ):
            issues.append(
                Issue(
                    "E_CATEGORY",
                    f"{base}.category",
                    f"分类必须是非空、≤{MAX_CATEGORY_LEN} 字符、不含控制字符的字符串"
                    f"（推荐九类: {CATEGORIES}，允许自定义）",
                )
            )
