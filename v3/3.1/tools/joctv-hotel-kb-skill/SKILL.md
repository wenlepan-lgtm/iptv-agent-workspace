...
```bash
# 1. 生成 (确定性: 同包同 seed 同字节输出; 每条问法最终路由硬断言, 违规即失败)
python3 tools/joctv-hotel-kb-skill/scripts/generate_utterance_coverage.py \
    --package <kb.json> --out <outdir> [--seed 20260904] [--per-cell 40] \
    [--clarify-per-cell 8]
...
两层口径，算法副本集中在 `scripts/kb_router_chain.py`（与生成器硬断言/校验器 E_ROUTE_CONSISTENCY 共用同一实现，**不存在第二套未建模的决策副本**），内置源码快照 SHA 防漂移门（网关算法变化时评测 FAIL，需重对齐副本）；Skill 单独拷贝到工作站时用 `--allow-missing-router` 跳过。
